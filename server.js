...
const CompanyRevenueSchema = new mongoose.Schema({
  source: {
    type: String,
    enum: ['loan_interest', 'investment_fee', 'withdrawal_fee', 'platform_fee', 'disbursement_fee'],
    required: true,
    index: true
  },
  amount: {
...
CompanyRevenueSchema.index({ source: 1, recordedAt: -1 });
CompanyRevenueSchema.index({ userId: 1, recordedAt: -1 });
CompanyRevenueSchema.index({ loanId: 1, recordedAt: -1 });
...
const LoanSchema = new mongoose.Schema({
  ...
  status: { 
    type: String, 
    enum: ['draft', 'pending', 'under_review', 'approved', 'rejected', 'active', 'completed', 'defaulted', 'cancelled'],
    default: 'draft',
    index: true
  },
  statusHistory: [{
  ...
  purpose: {
    type: String,
    enum: ['cloud_mining_expansion', 'equipment_upgrade', 'portfolio_diversification', 'emergency_funds', 'other'],
    required: true
  },
...
LoanSchema.virtual('isOverdue').get(function() {
  if (!this.nextPaymentDate || this.status !== 'active') return false;
...
LoanSchema.pre('save', function(next) {
  // Generate loan number if new
  if (this.isNew && !this.loanNumber) {
    this.loanNumber = `LN-${Date.now()}-${Math.floor(Math.random() * 1000)}`;
  }
  
  // Calculate amount due
  if (this.totalRepayment && this.amountPaid) {
    this.amountDue = Math.max(0, this.totalRepayment - this.amountPaid);
  }
...
// services/LoanEligibilityService.js

class LoanEligibilityService {
  constructor() {
    this.MIN_LOAN_AMOUNT = 1000;
    this.MAX_LOAN_AMOUNT = 50000;
    this.MIN_INVESTMENTS = 5;
    this.MIN_INVESTMENT_TOTAL = 1000;
    this.LEVERAGE_RATIO = 3; // 3x main balance
    this.MIN_CREDIT_SCORE = 600;
  }
  ...
  async checkEligibility(userId, requestedAmount) {
    try {
      ...
      const checks = [
        ...
        this.checkActiveLoans(user),
        this.checkInvestmentRequirements(user),
        this.checkLoanAmount(requestedAmount, user.balances.main),
        await this.checkCreditScore(userId)
      ];
      ...
      if (failedChecks.length > 0) {
        return {
          ...
          requirements: this.getRequirements(user)
        };
      }

      const eligibilityScore = await this.calculateEligibilityScore(userId, requestedAmount);
      const maxLoanAmount = this.calculateMaxLoanAmount(user.balances.main);
      ...
      return {
        ...
        requirements: this.getRequirements(user),
        terms: this.getLoanTerms(requestedAmount)
      };
  ...
  async getUserData(userId) {
    const [user, activeInvestments, activeLoans] = await Promise.all([
      User.findById(userId)
        .select('balances status kycStatus firstName lastName email createdAt')
        .lean(),
      Investment.find({ 
        user: userId, 
        status: 'active' 
      }).select('amount startDate').lean(),
      Loan.find({ 
        user: userId, 
  ...
  checkAccountStatus(user) {
    const passed = user.status === 'active';
    return {
      passed,
      reason: passed ? null : 'Account is not active or suspended',
      code: 'ACCOUNT_STATUS'
    };
  }
  ...
  checkKYCStatus(user) {
    ...
    const passed = kyc.identity === 'verified' && 
                   kyc.address === 'verified' && 
                   kyc.facial === 'verified';
    
    return {
      passed,
      reason: passed ? null : 'Complete KYC verification (Identity, Address, Facial)',
      code: 'KYC_STATUS'
    };
  }

  checkActiveLoans(user) {
    ...
    return {
      passed,
      reason: passed ? null : `You have ${activeLoans.length} active/pending loan(s)`,
      code: 'ACTIVE_LOANS',
      details: activeLoans
    };
  }

  checkInvestmentRequirements(user) {
    const investments = user.investments || [];
    const totalInvestment = investments.reduce((sum, inv) => sum + (inv.amount || 0), 0);
    
    const passed = investments.length >= this.MIN_INVESTMENTS && 
                   totalInvestment >= this.MIN_INVESTMENT_TOTAL;
    
    return {
      passed,
      reason: passed ? null : `Need ${this.MIN_INVESTMENTS}+ investments totaling $${this.MIN_INVESTMENT_TOTAL}+`,
      code: 'INVESTMENT_REQUIREMENTS',
      details: {
        count: investments.length,
        total: totalInvestment,
        requiredCount: this.MIN_INVESTMENTS,
        requiredTotal: this.MIN_INVESTMENT_TOTAL
      }
    };
  }

  checkLoanAmount(requestedAmount, mainBalance) {
    const maxAllowed = mainBalance * this.LEVERAGE_RATIO;
    
    if (requestedAmount < this.MIN_LOAN_AMOUNT) {
      return {
        passed: false,
        reason: `Minimum loan amount is $${this.MIN_LOAN_AMOUNT}`,
    ...
    if (requestedAmount > maxAllowed) {
      return {
        passed: false,
        reason: `Maximum loan amount is $${maxAllowed.toFixed(2)} (${this.LEVERAGE_RATIO}x your main balance)`,
        code: 'MAX_LOAN_AMOUNT',
        details: { maxAllowed, mainBalance, leverage: this.LEVERAGE_RATIO }
      };
    }
  ...
  async checkCreditScore(userId) {
    ...
    const passed = creditScore >= this.MIN_CREDIT_SCORE;
    
    return {
      passed,
      reason: passed ? null : `Minimum credit score of ${this.MIN_CREDIT_SCORE} required`,
      code: 'CREDIT_SCORE',
      details: { score: creditScore, minimum: this.MIN_CREDIT_SCORE }
    };
  }
  ...
  async calculateCreditScore(userId) {
    try {
      const [user, investments, transactions] = await Promise.all([
        ...
        Investment.find({ user: userId, status: 'completed' }).countDocuments(),
        Transaction.find({ 
          user: userId, 
          status: 'completed',
          type: { $in: ['deposit', 'investment', 'withdrawal'] }
      ...
      if (accountAgeDays > 180) score += 150;
      else if (accountAgeDays > 90) score += 100;
      else if (accountAgeDays > 30) score += 50;

      // Successful investments (max +100)
      if (investments > 10) score += 100;
      else if (investments > 5) score += 50;

      // Transaction consistency (max +50)
      if (transactions.length > 20) score += 50;
      else if (transactions.length > 10) score += 25;

      // Recent activity bonus (max +50)
      const recentTransactions = transactions.filter(t => 
        Date.now() - new Date(t.createdAt).getTime() < 30 * 24 * 60 * 60 * 1000
      );
      if (recentTransactions.length > 5) score += 50;
  ...
  calculateMaxLoanAmount(mainBalance) {
    return mainBalance * this.LEVERAGE_RATIO;
  }

  async calculateEligibilityScore(userId, requestedAmount) {
    const creditScore = await this.calculateCreditScore(userId);
    const user = await this.getUserData(userId);
    
    let score = creditScore * 0.7; // 70% weight to credit score
    
    // Investment activity bonus (15%)
    const investmentTotal = user.investments?.reduce((sum, inv) => sum + (inv.amount || 0), 0) || 0;
    score += (investmentTotal / 10000) * 150; // Up to 15 points per $10k
    
    // Account age bonus (10%)
    const accountAgeDays = Math.floor((Date.now() - new Date(user.createdAt)) / (1000 * 60 * 60 * 24));
    score += Math.min(100, accountAgeDays / 3); // Up to 100 points
    
    // Request amount penalty (5%)
    const maxAllowed = this.calculateMaxLoanAmount(user.balances.main);
    const amountRatio = requestedAmount / maxAllowed;
    if (amountRatio > 0.8) score -= 50; // Penalty for high utilization
    
    return Math.min(1000, Math.max(0, Math.round(score)));
  }

  getRequirements(user) {
    return {
      accountStatus: user.status === 'active',
      kycVerified: user.kycStatus?.identity === 'verified' && 
                   user.kycStatus?.address === 'verified' && 
                   user.kycStatus?.facial === 'verified',
      noActiveLoans: (user.loans?.length || 0) === 0,
      minInvestments: (user.investments?.length || 0) >= this.MIN_INVESTMENTS,
      minInvestmentAmount: (user.investments?.reduce((sum, inv) => sum + (inv.amount || 0), 0) || 0) >= this.MIN_INVESTMENT_TOTAL,
      mainBalance: user.balances?.main || 0
    };
  }

  getLoanTerms(requestedAmount) {
    const monthlyInterestRate = 9.99 / 100;
    
    return {
      interestRate: 9.99,
      disbursementFeePercentage: 0.99,
      disbursementFeeAmount: requestedAmount * 0.0099,
      netAmount: requestedAmount * 0.9901, // After 0.99% fee
      minTerm: 3,
      maxTerm: 24,
      minAmount: this.MIN_LOAN_AMOUNT,
      maxAmount: this.MAX_LOAN_AMOUNT,
      calculateMonthlyPayment: (term) => {
        if (term < 3 || term > 24) return 0;
        const monthly = (requestedAmount * monthlyInterestRate * Math.pow(1 + monthlyInterestRate, term)) / 
...
module.exports = new LoanEligibilityService();
...
// services/LoanRepaymentService.js

class LoanRepaymentService {
  constructor() {
    this.LATE_FEE_PERCENTAGE = 5; // 5% late fee
    this.MAX_DAYS_OVERDUE = 30;
  }

  async processAutomaticRepayment(userId, depositAmount, session = null) {
    const sessionOptions = session ? { session } : {};
    
    try {
      // Find active loan with optimistic locking
      ...
      if (!activeLoan) {
        return {
          success: false,
          message: 'No active loan with auto-debit enabled',
          depositRemaining: depositAmount
        };
      }

      const amountDue = activeLoan.amountDue || activeLoan.monthlyPayment;
      
      if (amountDue <= 0) {
        await this.markLoanAsCompleted(activeLoan, session);
        return {
          success: true,
          message: 'Loan already paid in full',
          depositRemaining: depositAmount,
          loanStatus: 'completed'
        };
      }

      // Check if deposit covers payment
      if (depositAmount < amountDue) {
        return {
          success: false,
          message: `Insufficient deposit. Need $${amountDue.toFixed(2)}, got $${depositAmount.toFixed(2)}`,
          requiredAmount: amountDue,
          depositRemaining: depositAmount
        };
      }
      ...
      await User.findByIdAndUpdate(
        ...
        {
          $inc: {
            'balances.main': depositAmount - amountDue
          }
        },
      ...
      return {
        success: true,
        message: 'Automatic loan repayment successful',
        paymentDetails: paymentResult,
        depositRemaining: depositAmount - amountDue,
        loanStatus: paymentResult.newLoanStatus
      };

    } catch (error) {
      console.error('Automatic repayment error:', error);
      throw error;
    }
  }

  async processPayment(loan, amount, paymentType = 'manual', session = null) {
    const sessionOptions = session ? { session } : {};
    ...
    try {
      ...
      try {
        ...
        const scheduleItem = loan.repaymentSchedule.find(item => 
          item.status === 'pending' && 
        ...
        // Save loan
        await loan.save({ ...sessionOptions, session: internalSession });
        ...
        const transaction = await Transaction.create([{
          ...
          status: 'completed',
          method: 'internal',
          reference: `LOAN-PAY-${Date.now()}-${Math.floor(Math.random() * 1000)}`,
          details: {
            loanId: loan._id,
            loanNumber: loan.loanNumber,
            paymentType: paymentType,
            principal: amount * 0.7, // Estimated 70% principal
            interest: amount * 0.3,  // Estimated 30% interest
            remainingBalance: Math.max(0, loan.totalRepayment - loan.amountPaid)
          },
          netAmount: -amount,
          metadata: {
            autoDebit: paymentType === 'automatic'
          }
        }], { session: internalSession });

        // Record company revenue from interest
        const interestAmount = amount * 0.3; // 30% of payment is interest
        await CompanyRevenue.create([{
          ...
          description: `Loan interest payment - ${paymentType}`,
          metadata: {
        ...
        await User.findByIdAndUpdate(
          ...
          {
            $inc: {
              'balances.loan': -amount
            }
          },
          { ...sessionOptions, session: internalSession }
        );
  ...
  async markLoanAsCompleted(loan, session = null) {
    loan.status = 'completed';
    loan.endDate = new Date();
    loan.amountPaid = loan.totalRepayment;
    loan.amountDue = 0;
    
    await loan.save(session ? { session } : {});
  }
    ...
    try {
      ...
      return {
        ...
        overdue: overdueLoans.map(loan => ({
          ...
          loanNumber: loan.loanNumber,
          userId: loan.user,
          daysOverdue: Math.floor((new Date() - loan.nextPaymentDate) / (1000 * 60 * 60 * 24))
        }))
      };

    } catch (error) {
      console.error('Overdue loan check error:', error);
      throw error;
    }
  }

  async applyLateFee(loan) {
    const daysOverdue = Math.floor((new Date() - loan.nextPaymentDate) / (1000 * 60 * 60 * 24));
    
    if (daysOverdue > 0 && loan.lateFees === 0) {
      const lateFee = loan.monthlyPayment * (this.LATE_FEE_PERCENTAGE / 100);
      
      loan.lateFees = lateFee;
      loan.totalRepayment += lateFee;
      ...
      await Transaction.create({
        ...
        status: 'completed',
        method: 'internal',
        reference: `LATE-FEE-${Date.now()}`,
        details: {
          loanId: loan._id,
          loanNumber: loan.loanNumber,
          daysOverdue: daysOverdue,
          feePercentage: this.LATE_FEE_PERCENTAGE
        },
        netAmount: -lateFee
      });
    }
  ...
  async sendOverdueNotification(loan) {
    ...
    await sendProfessionalEmail({
      ...
      data: {
        ...
        lateFee: loan.lateFees
      }
    });
  }
}

module.exports = new LoanRepaymentService();
...
// @desc    Check user eligibility for loan
// @access  Private
app.post('/api/loans/check-eligibility', 
  protect,
  [
    ...
    body('term')
      .optional()
      .isInt({ min: 3, max: 24 })
      .withMessage('Term must be between 3-24 months')
  ],
  async (req, res) => {
    try {
      const errors = validationResult(req);
      ...
      const { requestedAmount, term = 12 } = req.body;
      const userId = req.user._id;

      // Check eligibility
      const eligibility = await LoanEligibilityService.checkEligibility(userId, requestedAmount);
      ...
      const terms = LoanEligibilityService.getLoanTerms(requestedAmount);
      const monthlyPayment = terms.calculateMonthlyPayment(term);
      ...
      res.status(200).json({
        ...
        data: {
          ...
          loanTerms: {
            ...
            disbursementFee: terms.disbursementFeeAmount,
            term: term,
            interestRate: terms.interestRate,
            monthlyPayment: monthlyPayment,
            totalRepayment: totalRepayment,
            totalInterest: totalInterest,
            apr: ((totalInterest / requestedAmount) * (12 / term) * 100).toFixed(2),
            startDate: new Date().toISOString(),
            endDate: new Date(Date.now() + term * 30 * 24 * 60 * 60 * 1000).toISOString()
          },
          documentsRequired: [
        ...
        metadata: {
          service: 'loan-eligibility',
          version: '1.0',
          cacheKey: `eligibility:${userId}:${requestedAmount}`,
          expiresAt: new Date(Date.now() + 300000).toISOString() // 5 minutes cache
        }
      });

    } catch (error) {
      console.error('Loan eligibility endpoint error:', error);

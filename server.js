...
// Admin - Add balance to user (COMPATIBLE WITH FRONTEND)
app.post('/api/admin/users/:userId/balance', adminProtect, restrictTo('super', 'finance'), [
  ...
  try {
    ...
    // Validate balance type exists
    if (!user.balances.hasOwnProperty(balanceType)) {
      return res.status(400).json({
        status: 'fail',
        message: `Invalid balance type: ${balanceType}`
      });
    }
    ...
    // Add to the specified balance with proper decimal handling
    user.balances[balanceType] = parseFloat((previousBalance + amountValue).toFixed(2));
    
    // Save user with updated balance
    await user.save();

    // Create transaction record for audit trail
    const reference = `ADMIN-ADD-${Date.now()}-${Math.random().toString(36).substr(2, 9).toUpperCase()}`;
    ...
    const transaction = await Transaction.create({
      ...
      details: `Admin added $${amountValue.toFixed(2)} to ${balanceType} balance`,
    ...
    const responseData = {
      status: 'success',
      message: `Successfully added $${amountValue.toFixed(2)} to user's ${balanceType} balance`,
      data: {
        user: {
          id: user._id,
          email: user.email,
        ...
        balanceUpdate: {
          ...
          newBalance: parseFloat(user.balances[balanceType].toFixed(2)),
    ...
    res.status(500).json({
      status: 'error',
      message: 'An internal server error occurred while adding balance to user'
    });
  }
});

// Alternative simplified endpoint for testing
app.post('/api/admin/users/:userId/balance/simple', adminProtect, restrictTo('super', 'finance'), async (req, res) => {
  try {
    const { userId } = req.params;
    const { amount, balanceType, notes } = req.body;
    ...
    if (!balanceType || !['main', 'active', 'matured', 'savings', 'loan'].includes(balanceType)) {
      return res.status(400).json({
        status: 'fail',
        message: 'Valid balance type is required: main, active, matured, savings, or loan'
      });
    }
    ...
    // Initialize balances
    if (!user.balances) {
      user.balances = { main: 0, active: 0, matured: 0, savings: 0, loan: 0 };
    }
    ...
    console.error('Simple add balance error:', error);
    res.status(500).json({
      status: 'error',
...
// Debug endpoint to test the connection
app.get('/api/admin/users/:userId/balance/debug', adminProtect, async (req, res) => {
  try {
    const { userId } = req.params;

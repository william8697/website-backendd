...
// GET /api/loans/application-data - Get comprehensive loan application data
app.get('/api/loans/application-data', async (req, res) => {
  try {
    console.log('📋 Loan application data request received');
    ...
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
      return res.status(401).json({
        status: 'fail',
        message: 'Authentication required. Please login to apply for a loan.'
      });
    }
    ...
    const user = await User.findById(userId)
      .select('firstName lastName email balances kycStatus referralCode createdAt')
      .lean();
    ...
    const investments = await Investment.find({
      user: userId,
      status: 'active'
    }).select('amount plan startDate').lean();

    // Get user's active loans (debt)
    const activeLoans = await Loan.find({
      user: userId,
      status: { $in: ['active', 'pending'] }
    }).select('amount repaidAmount remainingBalance status startDate').lean();

    // Get user's total transaction history
    const transactionCount = await Transaction.countDocuments({ user: userId });

    // CALCULATE LOAN ELIGIBILITY DATA (matching frontend requirements)

    // 1. Calculate loan limit (3x main balance as per frontend requirement)
    const loanLimit = user.balances.main * 3;
    console.log(`💰 Loan limit calculated: $${user.balances.main} × 3 = $${loanLimit}`);

    // 2. Calculate total debt balance
    const debtBalance = activeLoans.reduce((total, loan) => {
      if (loan.status === 'active') {
        return total + (loan.remainingBalance || loan.amount);
      }
      return total;
    }, 0);
    console.log(`📉 Total debt balance: $${debtBalance}`);

    // 3. Calculate available credit
    const availableCredit = Math.max(0, loanLimit - debtBalance);
    console.log(`💳 Available credit: $${availableCredit}`);

    // 4. Calculate credit score (based on frontend requirements)
    let creditScore = 600; // Base score as shown in frontend
    
    // Add points based on investments
    if (investments.length >= 5) creditScore += 50;
    if (investments.length >= 10) creditScore += 75;
    
    // Add points based on investment amount
    const totalInvested = investments.reduce((sum, inv) => sum + (inv.amount || 0), 0);
    if (totalInvested >= 5000) creditScore += 50;
    if (totalInvested >= 10000) creditScore += 75;
    
    // Add points based on account age (older accounts get higher scores)
    const accountAgeMonths = (new Date() - new Date(user.createdAt)) / (1000 * 60 * 60 * 24 * 30);
    if (accountAgeMonths >= 3) creditScore += 25;
    if (accountAgeMonths >= 6) creditScore += 50;
    
    // Add points based on transaction history
    if (transactionCount >= 10) creditScore += 25;
    if (transactionCount >= 20) creditScore += 50;
    
    // Cap at 850 (standard FICO max)
    creditScore = Math.min(Math.max(creditScore, 300), 850);
    console.log(`🎯 Credit score calculated: ${Math.floor(creditScore)}`);

    // 5. Check eligibility requirements (as shown in frontend)
    const requirements = {
      // Requirement 1: Verified Account Status
      accountVerified: user.kycStatus?.identity === 'verified' && 
                      user.kycStatus?.address === 'verified',
      
      // Requirement 2: Minimum 5 Investments (from frontend calculator)
      minimumInvestments: investments.length >= 5,
      
      // Requirement 3: Good Transaction History
      goodTransactionHistory: transactionCount >= 10,
      
      // Requirement 4: Credit Score ≥ 600 (from frontend)
      creditScoreThreshold: creditScore >= 600
    };

    // Calculate overall eligibility
    const isEligible = requirements.accountVerified && 
                      requirements.minimumInvestments && 
                      requirements.goodTransactionHistory && 
                      requirements.creditScoreThreshold;

    console.log(`✅ Eligibility check: ${isEligible ? 'PASS' : 'FAIL'}`);
    console.log('📊 Requirements:', requirements);

    // 6. Loan terms (matching frontend display)
    const loanTerms = {
      interestRate: 9.99, // Monthly interest rate (as shown in frontend)
      disbursementFee: 0.99, // Disbursement fee percentage
      minAmount: 1000, // Minimum loan amount
      maxAmount: Math.min(availableCredit, 1000000), // Cap at $1M
      terms: [3, 6, 12, 24], // Available loan terms in months
      earlyRepaymentPenalty: 0, // No penalty for early repayment
      collateralRequired: true,
      collateralType: 'main_balance' // Using main balance as collateral
    };

    // 7. Investment statistics
    const investmentStats = {
      totalInvestments: investments.length,
      totalInvested: totalInvested,
      averageInvestment: investments.length > 0 ? totalInvested / investments.length : 0,
      activeInvestments: investments.length,
      investmentFrequency: investments.length / Math.max(1, accountAgeMonths)
    };

    // 8. Prepare response matching frontend expectations
    const response = {
      status: 'success',
      data: {
        // Basic user info
        ...
        // Requirements status (for frontend eligibility check display)
        requirements: {
          accountVerified: requirements.accountVerified,
          minimumInvestments: requirements.minimumInvestments,
          goodTransactionHistory: requirements.goodTransactionHistory,
          creditScoreThreshold: requirements.creditScoreThreshold
        },
        
        // Investment statistics
        investmentStats: investmentStats,
        
        // Loan terms (for frontend display)
        loanTerms: loanTerms,
        ...
        eligibilityStatus: {
          ...
          reasons: !isEligible ? [] : [
            ...(!requirements.accountVerified ? ['Account verification required'] : []),
            ...(!requirements.minimumInvestments ? ['Minimum 5 investments required'] : []),
            ...(!requirements.goodTransactionHistory ? ['Good transaction history required'] : []),
            ...(!requirements.creditScoreThreshold ? ['Credit score below 600'] : [])
          ]
        },
    ...
    console.log('✅ Loan application data prepared successfully');
    
    // Cache in Redis for 5 minutes
    const cacheKey = `loan_app_data:${userId}`;
    await redis.setex(cacheKey, 300, JSON.stringify(response));
    ...
    console.error('❌ Error fetching loan application data:', err);
    console.error('Error stack:', err.stack);
    
    // More detailed error response for debugging
    res.status(500).json({
      status: 'error',
      message: 'An error occurred while fetching loan application data',
      error: process.env.NODE_ENV === 'development' ? err.message : undefined,
      timestamp: new Date().toISOString()
    });
  }
...
function calculatePaymentSchedule(amount, interestRate, termMonths) {
  ...
  return {
    ...
    schedule: generateSchedule(amount, monthlyRate, termMonths, monthlyPayment)
  };
}

function generateSchedule(principal, monthlyRate, termMonths, monthlyPayment) {
  const schedule = [];
...
// Optional: Add a health check endpoint for loans
app.get('/api/loans/health', (req, res) => {
  res.status(200).json({
    status: 'success',
    message: 'Loan API is operational',
    timestamp: new Date().toISOString(),
    endpoints: {
      balances: '/api/loans/balances',
      eligibility: '/api/loans/check-eligibility',
      applicationData: '/api/loans/application-data',
...
app.post('/api/loans/check-eligibility', protect, async (req, res) => {
    try {
        ...
        const completedInvestments = investments.filter(inv => inv.status === 'completed');
        const investmentPerformance = completedInvestments.length > 0 ? 
            completedInvestments.reduce((sum, inv) => sum + (inv.actualReturn || 0), 0) / totalInvested * 100 : 0;
        ...
        // Add points for referral activity
        if (downlineRelationships.length > 0 || user.referredBy !== null) creditScore += 15;
        if (downlineRelationships.length >= 3) creditScore += 25;

        // Add points for commission history
        if (commissionHistory.length > 0) creditScore += 10;
        ...
        const activeUser = recentActivity >= 10;
        if (!activeUser) creditScore -= 30;
        ...
        // Criterion 5: Account Age
        const matureAccount = accountAgeMonths >= 1; // At least 1 month old

        // Criterion 6: Investment Performance
        const goodInvestmentPerformance = investmentPerformance >= 0; // Positive returns

        // Criterion 7: Credit Score Threshold
        const creditScoreThreshold = roundedCreditScore >= 600;

        // Criterion 8: Available Credit
        const sufficientAvailableCredit = requestedAmount <= availableCredit;
        ...
        // Create requirements array for frontend
        const requirements = [
            {
                name: 'Account Verification',
                met: accountVerified,
                description: accountVerified ? 'Account fully verified' : 'Account verification required'
            },
            {
                name: 'Minimum 5 Investments',
                met: minimumInvestments,
                description: minimumInvestments ? `You have ${investments.length} investments` : `You need ${5 - investments.length} more investments`
            },
            {
                name: 'Good Transaction History',
                met: goodTransactionHistory,
                description: goodTransactionHistory ? 'Good transaction history' : `Transaction success rate: ${transactionSuccessRate.toFixed(1)}%`
            },
            {
                name: 'Active Account',
                met: activeAccount,
                description: activeAccount ? 'Account is active' : 'Account must be active'
            },
            {
                name: 'Mature Account',
                met: matureAccount,
                description: matureAccount ? `Account age: ${Math.floor(accountAgeMonths)} months` : 'Account must be at least 1 month old'
            },
            {
                name: 'Positive Investment Performance',
                met: goodInvestmentPerformance,
                description: goodInvestmentPerformance ? `Investment performance: ${investmentPerformance.toFixed(1)}%` : 'Positive investment performance required'
            },
            {
                name: 'Credit Score ≥ 600',
                met: creditScoreThreshold,
                description: creditScoreThreshold ? `Credit score: ${roundedCreditScore}` : `Credit score: ${roundedCreditScore} (needs 600+)`
            },
            {
                name: 'Available Credit',
                met: sufficientAvailableCredit,
                description: sufficientAvailableCredit ? 
                    `Requested amount within limit` : 
                    `Requested amount ($${requestedAmount}) exceeds available credit ($${availableCredit.toFixed(2)})`
            }
        ];

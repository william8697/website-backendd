...
router.get('/api/markets', rateLimit({ windowMs: 15 * 1000, max: 30 }), async (req, res) => {
  try {
    ...
    // Cache for 1 minute
    await redis.set(cacheKey, JSON.stringify(marketData), 'EX', 60);
...
// Get top gainers
router.get('/api/markets/gainers', rateLimit({ windowMs: 30 * 1000, max: 20 }), async (req, res) => {
  try {
    const cacheKey = 'gainers_data';
    const cachedData = await redis.get(cacheKey);
    ...
    await redis.set(cacheKey, JSON.stringify({ gainers }), 'EX', 120);
...
// Get trending coins
router.get('/api/markets/trending', rateLimit({ windowMs: 30 * 1000, max: 20 }), async (req, res) => {
  try {
    ...
    await redis.set(cacheKey, JSON.stringify({ trending }), 'EX', 180);
...
// Execute trade
router.post('/api/trades/:type(buy|sell)', rateLimit({ windowMs: 60 * 1000, max: 10 }), authenticateJWT, async (req, res) => {
  try {
    ...
    if (user.balance < 100) {
      return res.status(400).json({ error: 'Insufficient balance. Please deposit.' });
    }
    ...
    if (type === 'buy' && user.balance < totalCost + fee) {
    ...
    const trade = new Trade({
      ...
      price,
      fee,
      total: type === 'buy' ? totalCost + fee : totalCost - fee,
      status: 'completed'
    });

    // Update user balance
    if (type === 'buy') {
      user.balance -= (totalCost + fee);
    } else {
      user.balance += (totalCost - fee);
    }

    // Update portfolio
    let portfolioItem = await Portfolio.findOne({ userId, coinId });
    
    if (type === 'buy') {
      if (portfolioItem) {
        portfolioItem.amount += amount;
        portfolioItem.avgPrice = ((portfolioItem.avgPrice * portfolioItem.amount) + (price * amount)) / (portfolioItem.amount + amount);
      } else {
    ...
    // Publish trade event to WebSocket
    if (wsServer) {
...
// Get futures pairs
router.get('/api/markets/futures', rateLimit({ windowMs: 30 * 1000, max: 30 }), async (req, res) => {
  try {
    const cacheKey = 'futures_pairs';
    const cachedData = await redis.get(cacheKey);
    ...
    // Get futures data from Bybit API
    const response = await axios.get('https://api.bybit.com/v2/public/tickers');
    
    const futures = response.data.result
      .filter(pair => pair.symbol.endsWith('USDT'))
      .map(pair => ({
        symbol: pair.symbol.replace('USDT', '/USDT'),
        price: pair.last_price,
        change24h: (pair.price_24h_pcnt * 100),
        fundingRate: pair.funding_rate * 100,
        volume24h: pair.volume_24h,
        high24h: pair.high_price_24h,
        low24h: pair.low_price_24h
      }));

    await redis.set(cacheKey, JSON.stringify(futures), 'EX', 60);
...
router.post('/api/futures/:type(long|short)', rateLimit({ windowMs: 60 * 1000, max: 10 }), authenticateJWT, async (req, res) => {
  try {
    ...
    if (user.balance < marginRequired) {
      return res.status(400).json({ 
        error: `Insufficient margin. Required: $${marginRequired.toFixed(2)}` 
      });
    }
...
// Get earn products
router.get('/api/earn/products', rateLimit({ windowMs: 60 * 1000, max: 20 }), async (req, res) => {
  try {
    const cacheKey = 'earn_products';
    const cachedData = await redis.get(cacheKey);
    ...
    const products = response.data.map(coin => ({
      ...
      image: coin.image,
      apy: (Math.random() * 25).toFixed(2), // Simulated APY
      duration: ['Flexible', '30d', '90d'][Math.floor(Math.random() * 3)],
      minDeposit: 10
    }));
    ...
    res.status(500).json({ error: 'Failed to fetch earn products' });
  }
});

// Stake assets
router.post('/api/earn/stake', rateLimit({ windowMs: 60 * 1000, max: 5 }), authenticateJWT, async (req, res) => {
  try {
    ...
    // Get product details
    const products = JSON.parse(await redis.get('earn_products')) || [];
    ...
    if (amount < product.minDeposit) {
      return res.status(400).json({ 
        error: `Minimum deposit is ${product.minDeposit} ${product.asset}` 
      });
    }

    // Check if user has enough balance
    const portfolioItem = await Portfolio.findOne({ userId, coinId: product.id });
    if (!portfolioItem || portfolioItem.amount < amount) {
    ...
    portfolioItem.amount -= amount;
    await Promise.all([stake.save(), portfolioItem.save()]);

    res.json({
      success: true,
      stake,
      portfolio: await Portfolio.find({ userId })
    });
  } catch (error) {
    console.error('Stake error:', error);
    res.status(500).json({ error: 'Failed to stake assets' });
  }
});
...
setInterval(async () => {
  try {
    ...
    const response = await axios.get('https://api.coingecko.com/api/v3/coins/markets', {
      params: {
        vs_currency: 'usd',
        ids: 'bitcoin,ethereum,binancecoin,ripple,cardano,solana,dogecoin,polkadot,shiba-inu',
        price_change_percentage: '24h'
      }
    });

    const priceUpdates = response.data.reduce((acc, coin) => {
      acc[coin.id] = {
    ...
    // Update Redis cache
    await redis.set('price_updates', JSON.stringify(priceUpdates));
  } catch (error) {

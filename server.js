...
// Add this in your server.js file after the other admin routes (around line 3600)
app.get('/api/admin/support/tickets/pending', adminProtect, restrictTo('super', 'support'), async (req, res) => {
    try {
        ...
        // Get pending tickets with user details
        ...
        await logActivity('view-pending-tickets', 'support-ticket', null, req.admin._id, 'Admin', req);
    } catch (err) {
        console.error('Get pending tickets error:', err);
        res.status(500).json({
            status: 'error',
            message: 'An error occurred while fetching pending tickets'
        });
    }
});
...
app.get('/api/admin/support/chat/conversations', adminProtect, restrictTo('super', 'support'), async (req, res) => {
    try {
        // Get all unique users who have created support tickets
        const conversations = await SupportTicket.aggregate([
            {
                $group: {
                    _id: "$user",
                    lastMessage: { $max: "$updatedAt" },
                    unreadCount: {
                        $sum: {
                            $cond: [
                                { $eq: ["$status", "pending"] },
        ...
        const formattedConversations = conversations.map(conv => ({
            user: {
                ...
                avatar: conv.user.avatar || 'https://www.dropbox.com/scl/fi/1dq16nex1borvvknpcwox/circular_dark_background.png?rlkey=sq2ujl2oxxk9vyvg1j7oz0cdb&raw=1'
            },
            last_message: {
                created_at: conv.lastMessage,
        ...
        await logActivity('view-chat-conversations', 'support-chat', null, req.admin._id, 'Admin', req);
    } catch (err) {
...
app.get('/api/admin/support/chat/messages/:userId', adminProtect, restrictTo('super', 'support'), async (req, res) => {
    try {
        ...
        // Get all tickets/messages from this user
        ...
        res.status(200).json({
            ...
            data: {
                user: {
                    ...
                    lastName: user.lastName,
                    email: user.email,
                    avatar: user.avatar || 'https://www.dropbox.com/scl/fi/1dq16nex1borvvknpcwox/circular_dark_background.png?rlkey=sq2ujl2oxxk9vyvg1j7oz0cdb&raw=1'
                },
                messages
            }
        });

        await logActivity('view-chat-messages', 'support-chat', userId, req.admin._id, 'Admin', req);
    } catch (err) {
...
app.post('/api/admin/support/chat/send', adminProtect, restrictTo('super', 'support'), [
    ...
    try {
        ...
        // Find the most recent open ticket from this user
        ...
        if (!ticket) {
            return res.status(404).json({
                status: 'fail',
                message: 'No open ticket found for this user'
            });
        }

        // Update the ticket with admin response
        ...
        // Create a new system notification for the user
        ...
        await logActivity('send-chat-message', 'support-chat', user_id, req.admin._id, 'Admin', req, { 
